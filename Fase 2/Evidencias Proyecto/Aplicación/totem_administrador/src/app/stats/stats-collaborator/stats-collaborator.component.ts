...
@Component({
  selector: 'app-stats-collaborator',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './stats-collaborator.component.html',
  styleUrls: ['./stats-collaborator.component.css']
})
export class StatsCollaboratorComponent implements OnInit {
  // Referencias a los gráficos
  @ViewChild('barChart') barChart: BaseChartDirective | undefined;

  // Datos y configuraciones para los gráficos
  mostSelectedCollaborator: string = ''; // Para almacenar el colaborador más seleccionado
  clickLogs: any[] = []; // Array para almacenar los logs de clics en la tabla
  pieClickLogs: any[] = [];
  barClickLogs: any[] = [];
  collaboratorClickCounts: { name: string, clicks: number }[] = [];
  startDate: string = '';
  endDate: string = '';
  searchLogs: any[] = []; 
  searchLogsPage: number = 1;
  clickLogsPage: number = 1;
  administrativePercentage: number = 0;
  teacherPercentage: number = 0;
  administrativeTotal: number = 0;
  teacherTotal: number = 0;
  totalClicks: number = 0; 
  currentPage: number = 1;
  itemsPerPage: number = 10;
  paginatedCollaborators: { name: string, clicks: number }[] = [];
...
getCollaboratorGroupData(): void {
  ...
  getBarChartData(): void {
    ...
    this.statsService.getClickLogs(startDateFormatted, endDateFormatted, 'collaborators', 'category')
      .subscribe(
        (response: any) => {
  ...
  calculateGroupPercentages(): void {
    const groupCounts: { [key: string]: number } = {};
    let total = 0;
  
    // Recorre los logs y cuenta cada grupo
    this.pieClickLogs.forEach(log => {
      const group = log.log_category_details;
      if (group) {
    ...
    // Calcula los porcentajes de cada grupo
    this.administrativePercentage = groupCounts['Administrativos'] ? parseFloat((groupCounts['Administrativos'] / total * 100).toFixed(1)) : 0;
    this.teacherPercentage = groupCounts['Docentes'] ? parseFloat((groupCounts['Docentes'] / total * 100).toFixed(1)) : 0;

      // Asigna la cantidad total de clics de cada grupo
    this.administrativeTotal = groupCounts['Administrativos'] || 0;
    this.teacherTotal = groupCounts['Docentes'] || 0;

    // Calcula el total de clics sumando ambos grupos
    this.totalClicks = this.administrativeTotal + this.teacherTotal;
  }
  ...
  formatName(name: string): string {
    ...
    return word.charAt(0).toUpperCase() + word.slice(1);
     ...
     getClickLogsForTable(): void {
      ...
      this.statsService.getClickLogs(startDateFormatted, endDateFormatted, 'collaborators', 'collaborator')
        .subscribe(
          (response: any) => {
            this.clickLogs = response.logs.map((log: any) => ({
              ...log,
              log_category_details: this.formatName(log.log_category_details)
            }));
          
            this.calculateCollaboratorClickCounts();
          },
          (error: any) => {
...
// Método para calcular el colaborador más seleccionado
calculateCollaboratorClickCounts(): void {
  const collaboratorCounts: { [key: string]: number } = {};

  // Contar las selecciones de cada colaborador
  this.clickLogs.forEach(log => {
    const collaborator = log.log_category_details; // Nombre del colaborador
    if (collaborator) {
      collaboratorCounts[collaborator] = (collaboratorCounts[collaborator] || 0) + 1;
    }
  });

  // Convertir el objeto de contadores en un array y ordenarlo
  this.collaboratorClickCounts = Object.keys(collaboratorCounts).map(key => ({
    name: key,
    clicks: collaboratorCounts[key]
  })).sort((a, b) => b.clicks - a.clicks); // Orden descendente por clics
...
// Método para ordenar los logs por fecha de la más reciente a la más antigua
  ...
  getSearchLogs(): void {
    ...
    this.statsService.getSearchLogs(startDateFormatted, endDateFormatted, 'collaborators', 'search')
      .subscribe(
        (response: any) => {
  ...
  // Método para obtener la lista paginada de colaboradores
  updatePaginatedCollaborators() {
    const start = (this.currentPage - 1) * this.itemsPerPage;
    const end = start + this.itemsPerPage;
    this.paginatedCollaborators = this.collaboratorClickCounts.slice(start, end);
  ...
  get totalPages() {
    return Math.ceil(this.collaboratorClickCounts.length / this.itemsPerPage);
  }
